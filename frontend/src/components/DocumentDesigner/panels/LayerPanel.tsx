...
const LayerPanel = ({
  ...
  return (
    <div className="flex w-64 shrink-0 flex-col rounded-xl border border-slate-200 bg-white">
      <div className="border-b border-slate-200 p-3">
        <p className="mb-2 text-xs font-semibold text-slate-500">এলিমেন্ট যোগ করুন</p>
          ...
          {ADD_LAYER_TYPES.map((type) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs text-slate-700 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
              >
                <Icon size={14} />
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <p className="mb-1 px-1 text-xs font-semibold text-slate-500">লেয়ার ({layers.length})</p>
        {ordered.length === 0 && <p className="px-1 py-4 text-center text-xs text-slate-400">কোনো লেয়ার নেই</p>}
            ...
            return (
              <li
                ...
                className={`cursor-pointer rounded-lg border px-2 py-1.5 text-xs transition ${
                  selected ? "border-blue-400 bg-blue-50" : "border-transparent hover:bg-slate-50"
                }`}
              >
                <div className="flex items-center gap-1.5">
                  <Icon size={13} className="shrink-0 text-slate-500" />
                  <span className="min-w-0 flex-1 truncate font-medium text-slate-700">{layerSummary(layer)}</span>
                </div>
                {selected && (

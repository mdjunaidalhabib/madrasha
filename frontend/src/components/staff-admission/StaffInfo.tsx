...
import { StaffFormData, StaffFormErrors } from "../../features/staff/StaffPage";
...
interface Props {
  ...
  setErrors?: React.Dispatch<React.SetStateAction<StaffFormErrors>>;
}
...
const StaffInfo: React.FC<Props> = ({ formData, setFormData, errors = {}, setErrors }) => {
  ...
  const ErrorText = ({ field }: { field: keyof StaffFormData }) =>
    errors[field] ? <p className="text-red-500 text-xs mt-1 dark:text-red-400">{errors[field]}</p> : null;

  const clearError = (field: keyof StaffFormData) => {
    if (!setErrors) return;
    setErrors((prev) => {
      const updated = { ...prev };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    clearError(name as keyof StaffFormData);
  };
    ...
    <div className="bg-white shadow-lg p-6 rounded-xl border border-gray-200 dark:bg-slate-900 dark:border-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="flex flex-col">
          ...
          <input name="name_bn" value={formData.name_bn || ""} onChange={handleChange} className={inputClass("name_bn")} />
        ...
        <div className="flex flex-col">
          ...
          <input name="name_ar" value={formData.name_ar || ""} onChange={handleChange} className={inputClass("name_ar")} />
        </div>

        {/* NID */}
        <div className="flex flex-col">
          <label className="text-sm font-medium text-gray-600 mb-1 dark:text-slate-400">NID</label>
          <input name="nid" value={formData.nid || ""} onChange={handleChange} className={inputClass("nid")} />
        </div>
        ...
        <div className="flex flex-col">
          ...
          <input name="phone" value={formData.phone || ""} onChange={handleChange} className={inputClass("phone")} />
        ...
        <ExperiencePicker
          ...
          onChange={(year, month) =>
            setFormData((prev) => ({ ...prev, experience_year: year, experience_month: month }))
          }
        />

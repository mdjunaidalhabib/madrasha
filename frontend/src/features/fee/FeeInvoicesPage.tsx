...
import { Search, Wallet, CircleCheck, X, HandCoins, Printer } from "lucide-react";
...
type StudentOption = { id: number; name_bn?: string; roll?: number };
...
const FeeInvoicesPage = () => {
  ...
  // Lightweight (no invoices/payments joined) list of every student, loaded
  // once, used purely to power the search-by-name/roll/id suggestions below.
  useEffect(() => {
    (async () => {
      try {
  ...
  const studentSuggestions = useMemo(() => {
    ...
    return allStudents
      .filter((s) => {
        ...
        return name.includes(q) || roll.includes(q) || id === q;
      })
      .slice(0, 8);
          ...
          {selectedStudent ? (
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm">
                ...
                <span className="text-gray-500">(রোল {selectedStudent.roll ?? "-"})</span>
              </div>
              <button
                type="button"
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="ছাত্রের নাম, রোল বা আইডি দিয়ে খুঁজুন"
                  ...
                  {studentSuggestions.length === 0 ? (
                    ...
                    studentSuggestions.map((s) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-400">রোল {s.roll ?? "-"}</span>
                      </button>
                    ))
                  )}
                </div>
              ...
              {invoicesLoading ? (
                ...
                <>
                  <div className="mb-2 flex flex-wrap items-center justify-end gap-2">
                    {invoiceSummary.totalDue > 0 ? (
                      <button
                        type="button"
                        onClick={() => openPayModal(unpaidInvoices, selectedStudent.name_bn || "")}
                      ...
                      >
                        <Wallet size={13} />
                        সব বকেয়া ফি নিন (৳{invoiceSummary.totalDue.toLocaleString("bn-BD")})
                      </button>
                    ) : (
                        ...
                        >
                          ...
                          <div className="flex shrink-0 gap-1.5">
                            ...
                            {canAct && (
                              <button
                                type="button"
                                onClick={() => openPayModal([invoice], selectedStudent.name_bn || "")}

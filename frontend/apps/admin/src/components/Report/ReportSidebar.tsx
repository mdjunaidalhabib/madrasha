      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:block lg:space-y-2">
        {sections.map((section, sectionIndex) => (
          <div key={section.title || `general-${sectionIndex}`}>
            {section.title && (
              <div className="mb-1 mt-2 border-y border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[15px] font-semibold text-slate-800 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
                {section.title}
              </div>
            )}

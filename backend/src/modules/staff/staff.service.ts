...
const STAFF_NAME_FIELDS = {
  name_bn: "nameBn",
  name_ar: "nameAr",
  name_en: "nameEn",
} as const;

export type StaffNamesItem = { id: number } & Partial<Record<keyof typeof STAFF_NAME_FIELDS, string | null>>;
...
export class StaffService {
  ...
  /** নাম (৩ ভাষা) page: saves many staff members' name trios in one
   * transaction. Only nameBn/nameAr/nameEn are ever written; every id must
   * belong to this madrasa or nothing is saved. Mirrors
  ...
  async updateNamesBulk(madrasaId: number | undefined, items: StaffNamesItem[]) {
    ...
    if (missing.length) throw new BadRequestError(`Staff not found: ${missing.join(", ")}`);

    const updates = items.map((item) => {
      const data: Record<string, string | null> = {};
      for (const [key, column] of Object.entries(STAFF_NAME_FIELDS)) {
